...
export class ShowDeployment {
  ...
  protected readonly deploymentMode = new BehaviorSubject<
    'quick' | 'advanced' | null
  >(null);
  ...
  constructor() {
    this.gitRepositoryForm = this.formBuilder.group({
      repoUrl: [
        '',
        [Validators.required, Validators.pattern(/^https?:\/\/.*\.git$/)],
      ],
  ...
  protected startDeployment(): void {
    ...
    if (this.deploymentMode.value === 'quick') {
      console.log(
        'Starting quick deployment with:',
        this.gitRepositoryForm.value
      );
