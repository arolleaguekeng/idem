...
for COMPONENT_FILE in $COMPONENT_FILES; do
  ...
  CLASS_NAME="${BASE_NAME^}Component" # Première lettre en majuscule + "Component"
  NEW_CLASS_NAME="${BASE_NAME^}"      # Juste la première lettre en majuscule
...
for TS_FILE in $TS_FILES; do
  ...
  for COMPONENT_FILE in $COMPONENT_FILES; do
    FILENAME=$(basename "$COMPONENT_FILE")
    BASE_NAME=${FILENAME%.component.ts}
    CLASS_NAME="${BASE_NAME^}Component"
...
for HTML_FILE in $HTML_FILES; do
  # Mettre à jour les sélecteurs de composants (app-xxx-component -> app-xxx)
  for COMPONENT_FILE in $COMPONENT_FILES; do
    FILENAME=$(basename "$COMPONENT_FILE")
    BASE_NAME=${FILENAME%.component.ts}
    SELECTOR="app-$BASE_NAME"
    
    # Les sélecteurs de composants sont souvent au format app-xxx
    # Pas besoin de les changer car ils sont définis dans le @Component
  done
done
